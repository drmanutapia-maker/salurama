...
// Movido tal cual desde app/dashboard/seguridad/page.tsx (que ahora es solo
// un redirect a /dashboard/configuracion) -- misma lógica de sesiones +
// credenciales biométricas, sin cambios de comportamiento, adaptada a vivir
// como una sección dentro de la página nueva en vez de página completa: el
// check de sesión/redirect a /login ahora lo hace el padre (page.tsx) una
// sola vez para las 3 secciones, así que aquí ya no se repite.
...
export default function SeccionSeguridad() {
  ...
  const load = useCallback(async () => {
    ...
    // sección (se queda mostrando "Activar" por defecto, sin romper nada).
    await cargarEstadoBiometrico().catch(() => {})
    await cargarCredenciales().catch(() => {})
    if (!cancelRef.current) setLoading(false)
  }, [cargarSesiones, cargarEstadoBiometrico, cargarCredenciales])
  ...
  return (
    <section style={{ marginBottom: 40 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
        <ShieldCheck size={20} color="#1E3A5F" aria-hidden="true" />
        <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: 18, fontWeight: 900, color: '#1E3A5F' }}>Seguridad</h2>
      </div>
      ...
      {loadError ? (
        ...
        <>
          ...
          <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #E5E7EB', overflow: 'hidden' }}>
            {construirTarjetas(sessions, credenciales).length === 0 && (
              <p style={{ padding: 24, color: '#6B7280', fontSize: 14, textAlign: 'center' }}>No hay dispositivos que mostrar.</p>
            )}
            {construirTarjetas(sessions, credenciales).map((t, i) => {
...
// Skeleton de la sección de Seguridad — misma estructura de lista de
// tarjetas (avatar + nombre + acciones) para que la carga no salte.
function SeguridadSkeleton() {
  return (
    <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #E5E7EB', overflow: 'hidden' }} aria-busy="true">
