-- Corrige audit_trigger_function para tablas sin tenant_id directo.
-- Hace un SELECT al padre cuando la tabla auditada no tiene la columna tenant_id.
-- Tablas cubiertas:
--   patient_measurements  → hema.patients.tenant_id  (via patient_id)
--   cumulative_doses      → hema.patients.tenant_id  (via patient_id)
--   order_drugs           → hema.orders.tenant_id    (via order_id)
--   signatures            → hema.orders.tenant_id    (via order_id)
--   lab_values            → hema.lab_panels.tenant_id (via panel_id)
...
BEGIN
  ...
  IF v_tenant_id IS NULL THEN
    CASE TG_TABLE_NAME
      WHEN 'patient_measurements', 'cumulative_doses' THEN
