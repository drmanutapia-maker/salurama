...
// TOP 10 MÁS BUSCADAS EN MÉXICO 2026
const MAS_BUSCADAS = [
  'Oncología',
  'Cardiología',
  'Dermatología',
...
const ESPECIALIDADES_CONACEM = [
  ...
  'Oncología Radioterápica','Patología','Pediatría del Desarrollo y Conducta',
]
...
export default function HomePage() {
  ...
  useEffect(() => {
    async function loadEspecialidades() {
      try {
        ...
        setEspecialidades([...ESPECIALIDADES_CONACEM,...extras.sort()])
      } catch (error) {
  ...
  useEffect(() => {
    async function loadMedicos() {
      try {
        const { data, error } = await supabase.from('doctors').select(`id, full_name, specialty, photo_url, location_city, location_state, clinic_name, consultation_price_general, whatsapp_available`).order('created_at', { ascending: false }).limit(50)
        if (error) throw error
        setMedicos(data || [])
      } catch (error) {
  ...
  useEffect(() => {
    if (inputValue.trim().length >= 2 && userTyped) {
      ...
      const sugerenciasFiltradas = especialidades.filter(esp => normalizarTexto(esp).includes(normalizarTexto(busquedaLower))).slice(0, 8)
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F9FAFB', fontFamily: "'DM Sans', sans-serif", color: '#111827' }}>
     ...
     .btn-violeta { background: #8B5CF6; color: white; border: none; transition: all 0.2s ease; }
     .btn-violeta:hover { background: #7C3AED; transform: translateY(-2px); box-shadow: 0 6px 20px rgba(139, 92, 246, 0.4); }
     .chip:hover { background: #8B5CF6; color: white; border-color: #8B5CF6; transform: translateY(-2px); }
     .card-medico { transition: all 0.3s ease; border: 1px solid #2A9D8F; }
     .card-medico:hover { transform: translateY(-4px); box-shadow: 0 12px 24px rgba(42, 157, 143, 0.15); border-color: #8B5CF6; }
      ...
      <header className="desktop-only" style={{ position: 'fixed', top: 0, left: 0, right: 0, background: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(12px)', borderBottom: '1px solid #E5E7EB', zIndex: 1000 }}>
        <div style={{ maxWidth: 1400, margin: '0 auto', padding: '16px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 40 }}>
          ...
          <nav style={{ display: 'flex', alignItems: 'center', gap: 40 }}>
            <Link href="/buscar" className="nav-link" style={{ color: '#4A5568', textDecoration: 'none', fontSize: 14, fontWeight: 500 }}>Especialidades</Link>
            <Link href="/como-elegir-medico" className="nav-link" style={{ color: '#4A5568', textDecoration: 'none', fontSize: 14, fontWeight: 500 }}>¿Cómo elegir médico?</Link>
            <Link href="/nosotros" className="nav-link" style={{ color: '#4A5568', textDecoration: 'none', fontSize: 14, fontWeight: 500 }}>Nosotros</Link>
          </nav>
          <div ref={dropdownRef} className="doctor-dropdown" style={{ position: 'relative' }}>
            <button onClick={() => setDoctorDropdownOpen(!doctorDropdownOpen)} style={{ background: 'linear-gradient(135deg, #1E3A5F 0%, #1A3254 100%)', color: '#fff', padding: '10px 24px', borderRadius: 50, fontSize: 14, fontWeight: 600, border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, transition: 'all 0.2s' }}>
              Soy Médico<ChevronDown size={16} style={{ transition: 'transform 0.2s', transform: doctorDropdownOpen? 'rotate(180deg)' : 'rotate(0deg)' }} />
            </button>
            {doctorDropdownOpen && (
              <div className="fade-in" style={{ position: 'absolute', top: '100%', right: 0, marginTop: 8, background: '#fff', borderRadius: 12, border: '1px solid #E5E7EB', boxShadow: '0 10px 40px rgba(30, 58, 95, 0.15)', minWidth: 200, overflow: 'hidden', zIndex: 1001 }}>
                <Link href="/login" onClick={() => setDoctorDropdownOpen(false)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', color: '#1E3A5F', textDecoration: 'none', fontSize: 14, fontWeight: 500, borderBottom: '1px solid #F3F4F6', transition: 'background 0.2s' }} onMouseEnter={(e) => { e.currentTarget.style.background = '#F5F3FF'; e.currentTarget.style.color = '#8B5CF6' }} onMouseLeave={(e) => { e.currentTarget.style.background = '#fff'; e.currentTarget.style.color = '#1E3A5F' }}>
                  <LogIn size={18} />Iniciar sesión
                </Link>
                <Link href="/registro" onClick={() => setDoctorDropdownOpen(false)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', color: '#1E3A5F', textDecoration: 'none', fontSize: 14, fontWeight: 500, transition: 'background 0.2s' }} onMouseEnter={(e) => { e.currentTarget.style.background = '#F5F3FF'; e.currentTarget.style.color = '#8B5CF6' }} onMouseLeave={(e) => { e.currentTarget.style.background = '#fff'; e.currentTarget.style.color = '#1E3A5F' }}>
      ...
      <main style={{ paddingTop: 80, paddingBottom: 100 }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 20px' }}>
          <section className="fade-up" style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 20px 30px', textAlign: 'center' }}>
            ...
            <h1 style={{ fontSize: 'clamp(24px, 5vw, 40px)', fontWeight: 900, color: '#1E3A5F', marginBottom: 32, lineHeight: 1.2 }}>Verifica credenciales y agenda con confianza</h1>
            <div className="sugerencias-wrapper" style={{ maxWidth: 800, margin: '0 auto 32px', display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              <div style={{ flex: '1 1 300px', position: 'relative' }}>
                <input type="text" placeholder="Especialidad o médico..." value={inputValue} onChange={handleInputChange} onKeyDown={(e) => { if (e.key === 'Enter') { handleSearch() } }} style={{ width: '100%', padding: '18px 140px 18px 24px', borderRadius: 16, border: '1.5px solid #2A9D8F', fontSize: 15, background: '#fff', outline: 'none' }} />
                <button onClick={handleSearch} style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', background: '#8B5CF6', border: 'none', borderRadius: 12, padding: '10px 20px', color: 'white', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                {showSugerencias && (
                  <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: 4, background: '#fff', borderRadius: 12, border: '1px solid #E5E7EB', boxShadow: '0 8px 24px rgba(0,0,0,0.12)', zIndex: 100, maxHeight: 300, overflow: 'auto' }}>
                    {sugerencias.map((sugerencia, index) => (
                      <button key={index} onClick={() => handleSugerenciaClick(sugerencia)} onMouseDown={(e) => { e.preventDefault(); handleSugerenciaClick(sugerencia) }} style={{ width: '100%', padding: '12px 16px', background: 'none', border: 'none', textAlign: 'left', fontSize: 14, cursor: 'pointer', transition: 'background 0.2s', borderBottom: index < sugerencias.length - 1? '1px solid #F3F4F6' : 'none' }} onMouseEnter={(e) => e.currentTarget.style.background = '#F5F3FF'} onMouseLeave={(e) => e.currentTarget.style.background = '#fff'}>
                        {sugerencia}
                      </button>
                    ))}
                  </div>
              ...
              <select value={selectedState} onChange={(e) => setSelectedState(e.target.value)} style={{ flex: '0 1 250px', padding: '18px 24px', borderRadius: 16, border: '1.5px solid #2A9D8F', fontSize: 15, fontFamily: "'DM Sans', sans-serif", background: '#fff', cursor: 'pointer', outline: 'none' }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, justifyContent: 'center', maxWidth: 900, margin: '0 auto' }}>
                {MAS_BUSCADAS.map((esp) => (
                  <button key={esp} onClick={() => { const params = new URLSearchParams(); params.set('especialidad', esp); if (selectedState) params.set('estado', selectedState); router.push(`/buscar?${params.toString()}`) }} className="chip" style={{ background: '#fff', border: '1px solid #2A9D8F', borderRadius: 50, padding: '8px 16px', fontSize: 13, color: '#4A5568', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>
                    {esp}
                  </button>
                ))}
              </div>
          ...
          <section className="fade-up" style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 20px', borderTop: '1px solid #E5E7EB' }}>
            ...
            {loading? (
              <div style={{ textAlign: 'center', padding: 60 }}><div style={{ width: 40, height: 40, border: '3px solid #E8ECF3', borderTopColor: '#8B5CF6', borderRadius: '50%', animation: 'spin 0.8s linear infinite', margin: '0 auto 16px' }} /><p style={{ color: '#9CA3AF', fontSize: 14 }}>Cargando médicos...</p></div>
            ) : medicos.length > 0? (
                ...
                {medicos.map((medico) => (
                  <Link key={medico.id} href={`/doctor/${medico.id}`} className="card-medico" style={{ background: '#fff', borderRadius: 16, padding: 20, textDecoration: 'none', display: 'flex', flexDirection: 'column', gap: 16 }}>
                    <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
                      {medico.photo_url? (<img src={medico.photo_url} alt={medico.full_name} style={{ width: 64, height: 64, borderRadius: '50%', objectFit: 'cover' }} />) : (<div style={{ width: 64, height: 64, borderRadius: '50%', background: 'linear-gradient(135deg, #1E3A5F, #2A9D8F)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, fontSize: 24, color: '#fff' }}>{(medico.full_name || '?')[0].toUpperCase()}</div>)}
                      <div style={{ flex: 1 }}><h3 style={{ fontSize: 16, fontWeight: 700, color: '#1E3A5F', marginBottom: 4 }}>{medico.full_name}</h3><p style={{ fontSize: 14, color: '#6B7280' }}>{medico.specialty}</p></div>
                    </div>
          ...
          <section className="fade-up" style={{ maxWidth: 1200, margin: '0 auto', padding: '60px 20px', borderTop: '1px solid #E5E7EB' }}>
