...
import { z } from 'zod'
import { sendCitaCanceladaPorMedicoEmail } from '@/lib/email'
...
// Mismo patrón que /api/citas/rechazar (que solo aplica a citas
// pending_verification) — esta ruta cubre el caso que ese endpoint
// rechaza explícitamente: cancelar una cita que el médico ya había
// confirmado. Antes esto se hacía con un UPDATE directo desde el cliente
// (cambiarEstado en dashboard/citas), sin capturar motivo ni avisar al
// paciente por correo.
export async function POST(request: NextRequest) {
  try {
    ...
    if (cita.estado !== 'confirmed') {
      return NextResponse.json({ error: 'Esta cita ya no se puede cancelar' }, { status: 400 })
    }
    ...
    if (updateError) {
      console.error('[cancelar-confirmada] Error actualizando cita:', updateError)
      return NextResponse.json({ error: 'Error al cancelar la cita' }, { status: 500 })
    }

    if (cita.paciente_email) {
      try {
        await sendCitaCanceladaPorMedicoEmail(
          cita.paciente_email,
        ...
        console.error('[cancelar-confirmada] Error enviando correo al paciente:', emailError)
      }
    }

    return NextResponse.json({ success: true })
  } catch (error) {
    console.error('[cancelar-confirmada] Error:', error)
