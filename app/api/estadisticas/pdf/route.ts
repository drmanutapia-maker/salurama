...
export async function GET() {
  ...
  try {
    const rateKey = `estadisticas_pdf:${user.id}`
    ...
    console.error('[estadisticas/pdf] Redis error:', redisError)
  }
  ...
  return new NextResponse(Buffer.from(pdfBytes), {
    headers: {
      ...
      'Content-Disposition': `attachment; filename="salurama-estadisticas-${new Date().toISOString().slice(0, 10)}.pdf"`,
