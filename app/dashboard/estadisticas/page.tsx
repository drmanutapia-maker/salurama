...
export default function EstadisticasPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: doc } = await supabase
        .from('doctors')
        .select('id, full_name, photo_url, specialty, about_me, clinic_address, whatsapp_available, availability_schedule, consultation_price_general, professional_license')
        .eq('email', user.email)
        .single()
      if (!doc) { router.push('/dashboard'); return }
      setMedico(doc)

      const [citasRes, reviewsRes] = await Promise.all([
  ...
  const campos = [
    ...
    { label: 'Sobre mí / Bio', ok: !!medico?.about_me },
    { label: 'Dirección del consultorio', ok: !!medico?.clinic_address },
    { label: 'WhatsApp disponible', ok: !!medico?.whatsapp_available },
    { label: 'Horario de disponibilidad', ok: !!medico?.availability_schedule },
    { label: 'Precio de consulta', ok: !!medico?.consultation_price_general },
    ...
    <div style={{ minHeight: '100vh', background: '#F9FAFB', fontFamily: "'DM Sans', sans-serif", color: '#1A1A2E' }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .section-title { font-family:'Fraunces',serif; font-size:16px; font-weight:900; color:#1A1A2E; margin-bottom:16px; }
      ...
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '24px 16px 80px' }}>
        ...
        <div className="fade-up" style={{ marginBottom: 28 }}>
          ...
          <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: 28, fontWeight: 900, color: '#1A1A2E', marginBottom: 4 }}>Estadísticas</h1>
        ...
        <div className="kpi-grid fade-up" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 20 }}>
          {[
            { icon: <Calendar size={20} color="#1E3A5F" />, label: 'Total citas', value: total, bg: '#EEF2FF', color: '#1E3A5F' },
            { icon: <CheckCircle size={20} color="#059669" />, label: 'Tasa confirmación', value: `${tasaConfirmacion}%`, bg: '#DCFCE7', color: '#059669' },
            { icon: <Star size={20} color="#D97706" />, label: 'Rating promedio', value: reviews.length > 0 ? ratingPromedio.toFixed(1) : '—', bg: '#FEF3C7', color: '#D97706' },
        ...
        {/* Estado de citas + Barras por mes */}
        <div className="fade-up" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 16, marginBottom: 16 }}>
            ...
            {total === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {[
                  ...
                  { label: 'Confirmadas', count: porStatus.confirmada, color: '#10B981' },
                  { label: 'Atendidas',   count: porStatus.atendida,  color: '#6366F1' },
          ...
          <div className="card">
            <p className="section-title" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <BarChart2 size={16} /> Últimos 6 meses
            </p>
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, height: 120 }}>
              {citasPorMes.map(({ label, count }) => (
                <div key={label} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, height: '100%', justifyContent: 'flex-end' }}>
                  ...
                  <div style={{ width: '100%', background: '#E0E7FF', borderRadius: '4px 4px 0 0', height: `${(count / maxMes) * 90}%`, minHeight: count > 0 ? 4 : 0, transformOrigin: 'bottom', animation: 'growY 0.6s ease' }} />
        ...
        {/* Día de semana + Reseñas distribución */}
        <div className="fade-up" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 16, marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, height: 100 }}>
              {citasPorDia.map(({ label, count }) => (
                <div key={label} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, height: '100%', justifyContent: 'flex-end' }}>
                  ...
                  <div style={{ width: '100%', background: '#CCFBF1', borderRadius: '3px 3px 0 0', height: `${(count / maxDia) * 80}%`, minHeight: count > 0 ? 4 : 0, animation: 'growY 0.6s ease', transformOrigin: 'bottom' }} />
                  <span style={{ fontSize: 10, color: '#9CA3AF', fontWeight: 600 }}>{label}</span>
                </div>
              ))}
            </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {ratingDist.map(({ stars, count, pct }) => (
                  <div key={stars} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: '#1A1A2E', minWidth: 12 }}>{stars}</span>
                    <Star size={12} color="#FBBF24" fill="#FBBF24" />
                    <div style={{ flex: 1, height: 8, background: '#F3F4F6', borderRadius: 4, overflow: 'hidden' }}>
                      <div style={{ height: '100%', width: `${pct}%`, background: '#FBBF24', borderRadius: 4, transition: 'width 0.6s ease' }} />
                    </div>
              ...
              {reviews.slice(0, 3).map(r => (
                <div key={r.id} style={{ background: '#F9FAFB', borderRadius: 10, padding: '14px 16px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                    <span style={{ fontSize: 13, fontWeight: 700, color: '#1A1A2E' }}>{r.user_name || 'Paciente'}</span>
                    <div style={{ display: 'flex', gap: 2 }}>
                      {Array.from({ length: 5 }).map((_, i) => (
                        <Star key={i} size={12} color="#FBBF24" fill={i < r.rating ? '#FBBF24' : 'none'} />
                      ))}
                    </div>
                  </div>
        ...
        <div className="card fade-up">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <p className="section-title" style={{ marginBottom: 0 }}>Completitud del perfil</p>
            <span style={{ fontFamily: "'Fraunces', serif", fontSize: 24, fontWeight: 900, color: completionPct >= 80 ? '#059669' : completionPct >= 50 ? '#D97706' : '#DC2626' }}>
              {completionPct}%
            </span>
          </div>
          <div style={{ height: 8, background: '#F3F4F6', borderRadius: 4, overflow: 'hidden', marginBottom: 16 }}>
            <div style={{ height: '100%', width: `${completionPct}%`, background: completionPct >= 80 ? '#10B981' : completionPct >= 50 ? '#F59E0B' : '#EF4444', borderRadius: 4, transition: 'width 0.8s ease' }} />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 8 }}>
            {campos.map(({ label, ok }) => (
              <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <div style={{ width: 18, height: 18, borderRadius: '50%', background: ok ? '#DCFCE7' : '#F3F4F6', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  {ok
                    ? <CheckCircle size={12} color="#059669" />
                    : <span style={{ width: 6, height: 6, background: '#D1D5DB', borderRadius: '50%' }} />}
                </div>
                <span style={{ fontSize: 13, color: ok ? '#1A1A2E' : '#9CA3AF', fontWeight: ok ? 500 : 400 }}>{label}</span>
              </div>
            ))}
          </div>
          {completionPct < 100 && (
