    ...
    <div style={{ minHeight: '100vh', background: '#F9FAFB', fontFamily: "'DM Sans', sans-serif", color: '#1A1A2E' }}>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '24px 16px 60px' }}>
        ...
        <div className="fade-up" style={{ marginBottom: 24 }}>
          <button onClick={() => router.push('/dashboard')} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'none', border: 'none', color: '#3730A3', fontSize: 14, fontWeight: 600, cursor: 'pointer', marginBottom: 16 }}>
            ← Volver al dashboard
          </button>
          <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: 28, fontWeight: 900, color: '#1A1A2E' }}>Editar perfil</h1>
        </div>
...
<Card title="Información para reservar por teléfono" onEdit={() => setActiveModal('booking')}>
  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
    ...
    {!(medico.consultation_price_general || 0) > 0 && 
     !Array.isArray(medico.payment_methods) && 
     !medico.whatsapp_available && 
     !medico.clinic_phone && (
      <p style={{ fontSize: 14, color: '#9CA3AF' }}>--</p>
    )}
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 12, padding: 20, border: '1.5px solid #E5E7EB' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
        ...
        <button onClick={onEdit} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: '#F9FAFB', border: '1.5px solid #E5E7EB', borderRadius: 8, padding: '6px 12px', fontSize: 13, fontWeight: 600, cursor: 'pointer', color: '#1A1A2E' }}>
...
function BasicInfoForm({ medico, onSave, saving }: { medico: Medico; onSave: (data: any) => void; saving: boolean }) {
  const [formData, setFormData] = useState({ full_name: medico.full_name, phone: medico.phone || '' })
  const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); onSave(formData) }
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div>
        <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Nombre completo</label>
        <input type="text" value={formData.full_name} onChange={(e) => setFormData({ ...formData, full_name: e.target.value })} style={{ width: '100%', padding: '10px 12px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }} required />
      </div>
      <div>
        <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Teléfono</label>
        <input type="tel" value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} style={{ width: '100%', padding: '10px 12px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }} />
      </div>
      <button type="submit" disabled={saving} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: saving ? '#9CA3AF' : '#3730A3', color: '#fff', border: 'none', borderRadius: 8, padding: '12px 20px', fontSize: 14, fontWeight: 600, cursor: saving ? 'not-allowed' : 'pointer' }}>
...
function IntroForm({ aboutMe, onSave, saving }: { aboutMe: string | null; onSave: (data: any) => void; saving: boolean }) {
  ...
  const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); onSave({ about_me: text }) }
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div>
        <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Sobre mí</label>
        <textarea value={text} onChange={(e) => setText(e.target.value)} rows={6} style={{ width: '100%', padding: '10px 12px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14, resize: 'vertical' }} placeholder="Cuéntales a los pacientes sobre ti..." />
      </div>
      <button type="submit" disabled={saving} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: saving ? '#9CA3AF' : '#3730A3', color: '#fff', border: 'none', borderRadius: 8, padding: '12px 20px', fontSize: 14, fontWeight: 600, cursor: saving ? 'not-allowed' : 'pointer' }}>
        <Save size={16} /> {saving ? 'Guardando...' : 'Guardar cambios'}
      </button>
...
function LicensesForm({ licenses, onAdd, onDelete, saving }: { licenses: License[]; onAdd: (data: any) => void; onDelete: (id: string) => void; saving: boolean }) {
  ...
  const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); onAdd(formData); setShowForm(false); setFormData({ license_number: '', license_type: 'profesional', institution: '', issue_date: '' }) }
  if (showForm) {
    return (
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div>
          <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Número de cédula *</label>
          <input type="text" value={formData.license_number} onChange={(e) => setFormData({ ...formData, license_number: e.target.value })} style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }} placeholder="Ej: 12345678" required />
        </div>
        <div>
          <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Tipo *</label>
          <select value={formData.license_type} onChange={(e) => setFormData({ ...formData, license_type: e.target.value })} style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }}>
        ...
        <div>
          ...
          <input type="text" value={formData.institution} onChange={(e) => setFormData({ ...formData, institution: e.target.value })} style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }} placeholder="Ej: UNAM" />
        </div>
        <div>
          <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Fecha de expedición</label>
          <input type="date" value={formData.issue_date} onChange={(e) => setFormData({ ...formData, issue_date: e.target.value })} style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }} />
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button type="submit" disabled={saving} style={{ flex: 1, background: '#3730A3', color: '#fff', border: 'none', borderRadius: 8, padding: '10px', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>{saving ? 'Guardando...' : 'Agregar'}</button>
          <button type="button" onClick={() => setShowForm(false)} style={{ flex: 1, background: '#F3F4F6', color: '#6B7280', border: 'none', borderRadius: 8, padding: '10px', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>Cancelar</button>
        </div>
      </form>
    )
  }
  return (
    <div>
      <p style={{ fontSize: 14, color: '#6B7280', marginBottom: 16 }}>Agrega tus cédulas para que los pacientes verifiquen tus credenciales</p>
      <button onClick={() => setShowForm(true)} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: '#EEF2FF', color: '#3730A3', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
...
function SpecialtiesForm({ specialty, onSave, saving }: { specialty: string; onSave: (data: any) => void; saving: boolean }) {
  ...
  const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); onSave({ specialty: text }) }
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div>
        <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Especialidad principal</label>
        <input type="text" value={text} onChange={(e) => setText(e.target.value)} style={{ width: '100%', padding: '10px 12px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }} required />
      </div>
      <button type="submit" disabled={saving} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: saving ? '#9CA3AF' : '#3730A3', color: '#fff', border: 'none', borderRadius: 8, padding: '12px 20px', fontSize: 14, fontWeight: 600, cursor: saving ? 'not-allowed' : 'pointer' }}>
        <Save size={16} /> {saving ? 'Guardando...' : 'Guardar cambios'}
      </button>
...
function ConditionsForm({ conditions, onAdd, onDelete, saving }: { conditions: Condition[]; onAdd: (data: any) => void; onDelete: (id: string) => void; saving: boolean }) {
  ...
  const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); onAdd(formData); setShowForm(false); setFormData({ condition_name: '', category: 'principal' }) }
  if (showForm) {
    return (
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div>
          <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Enfermedad o padecimiento *</label>
          <input type="text" value={formData.condition_name} onChange={(e) => setFormData({ ...formData, condition_name: e.target.value })} style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }} placeholder="Ej: Diabetes tipo 2" required />
        </div>
        <div>
          <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Categoría *</label>
          <select value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value })} style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }}>
  ...
  return (
    <div>
      <p style={{ fontSize: 14, color: '#6B7280', marginBottom: 16 }}>Agrega las enfermedades que tratas para ayudar a los pacientes a encontrarte</p>
      <button onClick={() => setShowForm(true)} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: '#EEF2FF', color: '#3730A3', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
...
function ExperienceForm({ experience, onAdd, onDelete, saving }: { experience: Experience[]; onAdd: (data: any) => void; onDelete: (id: string) => void; saving: boolean }) {
  ...
  const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); onAdd(formData); setShowForm(false); setFormData({ institution_name: '', position: '', location: '', start_date: '', end_date: '', is_current: false }) }
  if (showForm) {
    return (
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div>
          <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Institución/Hospital *</label>
          <input type="text" value={formData.institution_name} onChange={(e) => setFormData({ ...formData, institution_name: e.target.value })} style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }} placeholder="Ej: Hospital Ángeles" required />
        </div>
        <div>
          <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Puesto</label>
          <input type="text" value={formData.position} onChange={(e) => setFormData({ ...formData, position: e.target.value })} style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }} placeholder="Ej: Médico adscrito" />
        </div>
        <div>
          <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Ubicación</label>
          <input type="text" value={formData.location} onChange={(e) => setFormData({ ...formData, location: e.target.value })} style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }} placeholder="Ej: Ciudad de México" />
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <div style={{ flex: 1 }}>
            <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Inicio</label>
            <input type="date" value={formData.start_date} onChange={(e) => setFormData({ ...formData, start_date: e.target.value })} style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }} />
          </div>
          <div style={{ flex: 1 }}>
            <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Fin</label>
            <input type="date" value={formData.end_date} onChange={(e) => setFormData({ ...formData, end_date: e.target.value })} style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }} disabled={formData.is_current} />
          </div>
        </div>
        <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}>
          <input type="checkbox" checked={formData.is_current} onChange={(e) => setFormData({ ...formData, is_current: e.target.checked })} /> Trabajo actual
        </label>
        <div style={{ display: 'flex', gap: 8 }}>
          <button type="submit" disabled={saving} style={{ flex: 1, background: '#3730A3', color: '#fff', border: 'none', borderRadius: 8, padding: '10px', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>{saving ? 'Guardando...' : 'Agregar'}</button>
          <button type="button" onClick={() => setShowForm(false)} style={{ flex: 1, background: '#F3F4F6', color: '#6B7280', border: 'none', borderRadius: 8, padding: '10px', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>Cancelar</button>
  ...
  return (
    <div>
      <p style={{ fontSize: 14, color: '#6B7280', marginBottom: 16 }}>Agrega tu experiencia en hospitales, clínicas, etc.</p>
      <button onClick={() => setShowForm(true)} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: '#EEF2FF', color: '#3730A3', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
...
function EducationForm({ education, onAdd, onDelete, saving }: { education: Education[]; onAdd: (data: any) => void; onDelete: (id: string) => void; saving: boolean }) {
  ...
  const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); onAdd(formData); setShowForm(false); setFormData({ institution: '', degree: '', field_of_study: '', graduation_year: new Date().getFullYear() }) }
  if (showForm) {
    return (
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div>
          <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Institución *</label>
          <input type="text" value={formData.institution} onChange={(e) => setFormData({ ...formData, institution: e.target.value })} style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }} placeholder="Ej: UNAM" required />
        </div>
        <div>
          <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Grado académico *</label>
          <select value={formData.degree} onChange={(e) => setFormData({ ...formData, degree: e.target.value })} style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }}>
        ...
        <div>
          ...
          <input type="text" value={formData.field_of_study} onChange={(e) => setFormData({ ...formData, field_of_study: e.target.value })} style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }} placeholder="Ej: Hematología" />
        </div>
        <div>
          <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Año de graduación</label>
          <input type="number" value={formData.graduation_year} onChange={(e) => setFormData({ ...formData, graduation_year: parseInt(e.target.value) })} style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }} min="1950" max={new Date().getFullYear()} />
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button type="submit" disabled={saving} style={{ flex: 1, background: '#3730A3', color: '#fff', border: 'none', borderRadius: 8, padding: '10px', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>{saving ? 'Guardando...' : 'Agregar'}</button>
          <button type="button" onClick={() => setShowForm(false)} style={{ flex: 1, background: '#F3F4F6', color: '#6B7280', border: 'none', borderRadius: 8, padding: '10px', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>Cancelar</button>
  ...
  return (
    <div>
      <p style={{ fontSize: 14, color: '#6B7280', marginBottom: 16 }}>Agrega tu formación académica</p>
      <button onClick={() => setShowForm(true)} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: '#EEF2FF', color: '#3730A3', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <button
        type="submit"
        disabled={saving}
        style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: saving ? '#9CA3AF' : '#3730A3', color: '#fff', border: 'none', borderRadius: 8, padding: '12px 20px', fontSize: 14, fontWeight: 600, cursor: saving ? 'not-allowed' : 'pointer' }}
      >
        <Save size={16} />
...
function SocialMediaForm({ socialMedia, onAdd, onDelete, saving }: { socialMedia: SocialMedia[]; onAdd: (data: any) => void; onDelete: (id: string) => void; saving: boolean }) {
  ...
  const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); onAdd(formData); setShowForm(false); setFormData({ platform: 'Facebook', url: '', username: '' }) }
  if (showForm) {
    return (
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div>
          <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Plataforma *</label>
          <select value={formData.platform} onChange={(e) => setFormData({ ...formData, platform: e.target.value })} style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }}>
            {PLATAFORMAS_SOCIALES.map(p => (<option key={p} value={p}>{p}</option>))}
          </select>
        </div>
        <div>
          <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>URL del perfil *</label>
          <input type="url" value={formData.url} onChange={(e) => setFormData({ ...formData, url: e.target.value })} style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }} placeholder="https://..." required />
        </div>
        <div>
          <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Nombre de usuario</label>
          <input type="text" value={formData.username} onChange={(e) => setFormData({ ...formData, username: e.target.value })} style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }} placeholder="Ej: @doctorjuan" />
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button type="submit" disabled={saving} style={{ flex: 1, background: '#3730A3', color: '#fff', border: 'none', borderRadius: 8, padding: '10px', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>{saving ? 'Guardando...' : 'Agregar'}</button>
          <button type="button" onClick={() => setShowForm(false)} style={{ flex: 1, background: '#F3F4F6', color: '#6B7280', border: 'none', borderRadius: 8, padding: '10px', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>Cancelar</button>
  ...
  return (
    <div>
      <p style={{ fontSize: 14, color: '#6B7280', marginBottom: 16 }}>Agrega tus redes sociales para que los pacientes te sigan</p>
      <button onClick={() => setShowForm(true)} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: '#EEF2FF', color: '#3730A3', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
    ...
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12 }}>
          <div>
            ...
            <input type="number" value={formData.consultation_price_general} onChange={(e) => setFormData({ ...formData, consultation_price_general: e.target.value })} style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }} placeholder="800" />
          </div>
          <div>
            <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Primera vez ($)</label>
            <input type="number" value={formData.consultation_price_first_time} onChange={(e) => setFormData({ ...formData, consultation_price_first_time: e.target.value })} style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }} placeholder="1000" />
          </div>
          <div>
            <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Seguimiento ($)</label>
            <input type="number" value={formData.consultation_price_followup} onChange={(e) => setFormData({ ...formData, consultation_price_followup: e.target.value })} style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }} placeholder="600" />
          </div>
        </div>
        <div style={{ marginTop: 12 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {METODOS_PAGO.map(method => (
              <label key={method} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, padding: '6px 12px', background: formData.payment_methods.includes(method) ? '#EEF2FF' : '#F9FAFB', borderRadius: 20, cursor: 'pointer', border: formData.payment_methods.includes(method) ? '1px solid #3730A3' : '1px solid #E5E7EB' }}>
                <input type="checkbox" checked={formData.payment_methods.includes(method)} onChange={() => togglePaymentMethod(method)} style={{ accentColor: '#3730A3' }} />
                {method}
              </label>
            ))}
          </div>
      ...
      <div>
        ...
        <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 14, marginBottom: 12 }}>
          <input type="checkbox" checked={formData.accepts_insurance} onChange={(e) => setFormData({ ...formData, accepts_insurance: e.target.checked })} style={{ accentColor: '#3730A3' }} />
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {SEGUROS_MEDICOS.map(seguro => (
                <label key={seguro} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, padding: '6px 12px', background: formData.insurance_names.includes(seguro) ? '#EEF2FF' : '#F9FAFB', borderRadius: 20, cursor: 'pointer', border: formData.insurance_names.includes(seguro) ? '1px solid #3730A3' : '1px solid #E5E7EB' }}>
                  <input type="checkbox" checked={formData.insurance_names.includes(seguro)} onChange={(e) => setFormData({ ...formData, insurance_names: e.target.checked ? [...formData.insurance_names, seguro] : formData.insurance_names.filter(s => s !== seguro) })} style={{ accentColor: '#3730A3' }} />
                  {seguro}
                </label>
              ))}
            </div>
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12 }}>
          <div>
            ...
            <input type="tel" value={formData.clinic_phone} onChange={(e) => setFormData({ ...formData, clinic_phone: e.target.value })} style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }} placeholder="55 1234 5678" />
          </div>
          <div>
            <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Mejor horario para llamar</label>
            <input type="text" value={formData.best_contact_time} onChange={(e) => setFormData({ ...formData, best_contact_time: e.target.value })} style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }} placeholder="9am - 6pm" />
          </div>
        </div>
        <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 14, marginTop: 12 }}>
          <input type="checkbox" checked={formData.whatsapp_available} onChange={(e) => setFormData({ ...formData, whatsapp_available: e.target.checked })} style={{ accentColor: '#25D366' }} />
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12 }}>
          <div>
            ...
            <input type="number" value={formData.min_patient_age} onChange={(e) => setFormData({ ...formData, min_patient_age: e.target.value })} style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }} placeholder="0" />
          </div>
          <div>
            <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Edad máxima (años)</label>
            <input type="number" value={formData.max_patient_age} onChange={(e) => setFormData({ ...formData, max_patient_age: e.target.value })} style={{ width: '100%', padding: '8px 10px', border: '1.5px solid #E5E7EB', borderRadius: 8, fontSize: 14 }} placeholder="100" />
          </div>
        </div>
      </div>

      {/* Botón Guardar */}
      <button type="submit" disabled={saving} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: saving ? '#9CA3AF' : '#3730A3', color: '#fff', border: 'none', borderRadius: 8, padding: '14px 24px', fontSize: 14, fontWeight: 600, cursor: saving ? 'not-allowed' : 'pointer', marginTop: 8 }}>
        <Save size={16} /> {saving ? 'Guardando...' : 'Guardar cambios'}
      </button>
    </form>
  )
