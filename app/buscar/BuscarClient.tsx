...
import {
  ...
  X, Navigation, GraduationCap, Globe, Loader2, Filter, BookOpen, ArrowRight,
} from 'lucide-react'
...
function BuscarContent({ initialMedicos, heroTitulo, heroTexto, articulosRelacionados, paginaDedicada }: { initialMedicos: Medico[]; heroTitulo: string; heroTexto: string | null; articulosRelacionados: BlogArticuloResumen[]; paginaDedicada: string | null }) {
  const searchParams = useSearchParams()
  const router = useRouter()

  // Data — llega ya resuelta desde el servidor (page.tsx) para que el HTML
  // inicial traiga la lista real, en vez de un skeleton vacío hasta que un
  // useEffect la pida. El filtro "cerca de mí" sigue reemplazando este
  // estado desde el cliente (necesita geolocalización del navegador).
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        const res = await fetch(`/api/buscar/sugerencias?q=${encodeURIComponent(inputValue.trim())}`, {
          signal: controller.signal,
        })
        const dbMatches: string[] = res.ok ? (await res.json()).especialidades ?? [] : []
    ...
    <div style={{ minHeight: '100vh', background: '#F9FAFB', fontFamily: "'DM Sans', sans-serif", color: '#111827' }}>
      ...
      <main style={{ paddingTop: 120, paddingBottom: 100 }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 20px' }}>
          ...
          {/* ── H1 + INTRO — server-rendered, arriba de la barra de búsqueda ── */}
          <div className="fade-up" style={{ textAlign: 'center', maxWidth: 720, margin: '0 auto 32px' }}>
            <h1 style={{ fontSize: 'clamp(24px, 4vw, 34px)', fontWeight: 900, color: '#1E3A5F', marginBottom: heroTexto ? 10 : 0 }}>
              {heroTitulo}
            </h1>
            {heroTexto && (
          ...
          {/* ── BANNER A PÁGINA DEDICADA — cuando esta combinación
              especialidad+estado ya calificó para /especialistas/..., el
              canonical de esta vista ya apunta ahí (Parte 3); este banner es
              el link visible que refuerza esa misma señal. ── */}
          {paginaDedicada && (
            <div className="fade-up" style={{ maxWidth: 720, margin: '0 auto 20px', textAlign: 'center' }}>
              <Link
                href={paginaDedicada}
                style={{
                  display: 'inline-flex', alignItems: 'center', gap: 8,
                  background: '#fff', border: '1.5px solid #8B5CF6', color: '#7C3AED',
                  borderRadius: 50, padding: '10px 20px', fontSize: 13.5, fontWeight: 700,
                  textDecoration: 'none',
                }}
              >
                Ver la página dedicada de esta especialidad y estado <ArrowRight size={14} />
              </Link>
            </div>
          )}

          {/* ── ARTÍCULOS RELACIONADOS — mitad "buscar → blog" del interlink
              bidireccional; la otra mitad es el CTA al fondo de cada
              artículo hacia /buscar?especialidad=X ── */}
          {articulosRelacionados.length > 0 && (
            <div className="fade-up" style={{ maxWidth: 720, margin: '0 auto 32px' }}>
                ...
                {articulosRelacionados.map(a => (
                  <Link
                    ...
                    style={{
                      ...
                      padding: '10px 16px', textDecoration: 'none', maxWidth: 320,
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 12, fontSize: 12.5, color: '#6B7280' }}>
                          <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                            ...
                            {medico.ciudad && medico.estado
                              ? `${medico.ciudad}, ${getStateLabel(medico.estado)}`
                              : medico.ciudad || (medico.estado ? getStateLabel(medico.estado) : '') || 'Ubicación no disponible'}
                          </span>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <p style={{ 
                  ...
                  {showMapModal.ciudad && showMapModal.estado
                    ? `${showMapModal.ciudad}, ${getStateLabel(showMapModal.estado)}`
                    : showMapModal.ciudad || (showMapModal.estado ? getStateLabel(showMapModal.estado) : '') || 'Ubicación no disponible'}
                </p>
              </div>
              <button
                onClick={() => setShowMapModal(null)}
...
export default function BuscarClient({ initialMedicos, heroTitulo, heroTexto, articulosRelacionados, paginaDedicada }: { initialMedicos: Medico[]; heroTitulo: string; heroTexto: string | null; articulosRelacionados: BlogArticuloResumen[]; paginaDedicada: string | null }) {
  return (
    <Suspense fallback={
      ...
      <BuscarContent initialMedicos={initialMedicos} heroTitulo={heroTitulo} heroTexto={heroTexto} articulosRelacionados={articulosRelacionados} paginaDedicada={paginaDedicada} />
    </Suspense>
  )
}