 ...
 *
 * Run:  node --use-system-ca scripts/ingest-msl-papers.js
 *
 * --use-system-ca es REQUERIDO en Windows con Node 24+.
 * Node 24 no confía en los certificados raíz del sistema operativo por
 * defecto, lo que provoca UNABLE_TO_VERIFY_LEAF_SIGNATURE en cualquier
 * llamada fetch() a dominios externos (Supabase, OpenAI).
 * En macOS / Linux / CI este flag es inofensivo — se puede incluir siempre.
 *
 * Compañero requerido: scripts/pdf-extract-worker.cjs  (extracción PDF)
 * Metadatos de papers:  data/msl-papers/papers.json
 */
...
function extractText(filePath) {
  ...
  if (ext === '.pdf') {
    const workerPath = path.join(__dirname, 'pdf-extract-worker.cjs');
...
async function main() {
  ...
  if (!fs.existsSync(PAPERS_DIR)) {
    ...
    console.log('📁 Carpeta creada. Agrega papers (.pdf / .txt) y vuelve a correr.\n');
    return;
  }

  const manifest = loadManifest();
  const files    = fs.readdirSync(PAPERS_DIR).filter(f => /\.(pdf|txt|md)$/i.test(f)).sort();

  if (files.length === 0) { console.log('⚠️  No hay papers en data/msl-papers/\n'); return; }
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    const { data: existing } = await supabase
      .from('msl_documents').select('id').eq('title', meta.title).maybeSingle();

    if (existing) { console.log('    ⏭️  Ya ingestado — omitiendo\n'); skipped++; continue; }

    let docId = null;
    try {
      process.stdout.write('    📄 Extrayendo texto... ');
      const text   = stripPdfBoilerplate(extractText(filePath));
      ...
      for (let b = 0; b < chunks.length; b += EMBED_BATCH) {
        const embs = await getEmbeddings(chunks.slice(b, b + EMBED_BATCH));
      ...
      if (docId) {
        await supabase.from('msl_documents').delete().eq('id', docId);
        docId = null;
      }
